"""
Progress Callback Interface Contract
=====================================

This contract defines the interface between BiasDetectionExperiment and progress consumers (TUI).

The experiment orchestrator accepts an optional callback implementing this protocol.
The TUI provides an implementation that writes to a queue for async UI updates.
...
# Example implementation for TUI (reference only, not contract)
class QueueProgressCallback:
    """
    Reference implementation that pushes events to a queue for async processing.

    This is NOT part of the contract - just an example of how TUI implements it.
    """

    def __init__(self, event_queue):
        self.queue = event_queue

    def on_experiment_start(self, session_id, config, total_phases):
        self.queue.put({
    ...
    def on_phase_start(self, phase_num, phase_name, items_total):
        self.queue.put({
    ...
    def on_progress(self, phase_num, items_done, items_total, message=None):
        self.queue.put({
    ...
    def on_phase_complete(self, phase_num, items_done, elapsed_seconds):
        self.queue.put({
    ...
    def on_phase_error(self, phase_num, error_type, error_message, traceback_str=None):
        self.queue.put({
    ...
    def on_experiment_complete(self, session_id, total_time_seconds, phases_completed):
        self.queue.put({
    ...
    def on_experiment_error(self, session_id, error_type, error_message, failed_phase):
        self.queue.put({
...
# Contract Test Spec (for pytest)
"""
Contract Test Requirements:
============================

1. Test that BiasDetectionExperiment accepts callback parameter:
   experiment = BiasDetectionExperiment(config, callback=mock_callback)

2. Test callback methods are called in correct order:
   - on_experiment_start (once at start)
   - on_phase_start (once per phase)
   - on_progress (multiple times per phase)
   - on_phase_complete (once per successful phase)
   - on_experiment_complete (once at end)

3. Test error path:
   - on_phase_error called when phase fails
   - on_experiment_error called when experiment fails
   - Partial results preserved

4. Test callback is optional:
   experiment = BiasDetectionExperiment(config)  # No callback = no error

5. Test thread safety:
   - Multiple rapid progress updates don't cause race conditions
   - Queue-based implementations don't drop events

6. Test backward compatibility:
   - Existing experiments without callback still work
   - CLI mode continues to function identically
"""
